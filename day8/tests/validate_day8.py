...
Usage: python tests/validate_day8.py  (from repo/day8/)
...
if os.path.exists(alert_file):
    try:
        with open(alert_file) as f:
            alerts = json.load(f)
        critical = alerts.get("critical", alerts) if isinstance(alerts, dict) else alerts
...
if results["fail"] == 0 and results["pass"] >= 12:
    ...
    print("  Then re-run: python tests/validate_day8.py\n")
